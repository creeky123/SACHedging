...
class Actor (nn.Module):
    ...
    def calculate_log_likelihood(self, obs, norm_dist):
        ...
        sum_log_tanh_sq = torch.log(torch.ones_like(power, device='cuda:0') - power + 0.00000002) #add some small number for numerical stability
...
class SoftActorCritic(nn.Module):
    def __init__(self, model_dict, state_space_size, action_space_size):
        ...
        self.polyak_average_params(self.Critic, self.TargetCritic, 1) #initialize the target and base critics to identical weights
    ...
    def train_one_step(self, batch, writer, n_steps):
        ...
        log_like_targ_entropy = (log_likelihood + self.TargetEntropy).detach() ## detach so that we don't calc gradient - throws error
        ...
        writer.add_scalar('Model Loss/Actor loss', actor_loss.item(), n_steps)
        writer.add_scalar('Model Loss/Critic 1 loss', q_1_loss.item(), n_steps)
        writer.add_scalar('Model Loss/Critic 2 loss', q_2_loss.item(), n_steps)
        writer.add_scalar('Model Loss/Alpha loss', alpha_loss.item(), n_steps)
        writer.add_scalar('Model Loss/Alpha', self.alpha.clone().item(), n_steps)
